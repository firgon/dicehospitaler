...
class DiceHospitalER extends Table
{
    ...
    */
    protected function getAllDatas()
    {
        ...
        $result['cards_count'] = $this->cards->countCardInLocation('deck') + 1; //count 1 card on 'table'
    ...
    function removeDupplicate($sets)
    {
        ...
        foreach ($sets as $set) {
            foreach ($set as $room) {
                ...
                $max = max($max, $counts[$room]);
            }
        }

        if ($max <= 1) return $sets;
        else {
            $set_to_erase = [];
            $max_score = 0;
            foreach ($sets as $set) {
    ...
    */
    ...
    function pass($die)
    {
        ...
        if ($score != -4) {
            ...
            $score = $score - 1;
            $nb_dead = -$score;
            $sql = "UPDATE player SET nb_deads = $nb_dead, score_dead = $score, player_score = score_wards+ score_critical+score_nurses + score_cardiologist_1 + score_cardiologist_2 + score_radiologist + score_epidemiologist + score_dead WHERE player_id=$player_id";
            self::DbQuery($sql);
            $loss = 1;
        }
    ...
    function play($die, $room_id, $value, $decoration, $bEndOfTurn)
    {
        ...
        if ($decoration != 'R' && $decoration != 'Y' && $decoration != 'G') {
            ...
            //TODO check if extra dice is correct
        ...
        if ($used_die != null && $this->powers[$used_die] == self::getGameStateValue('used_die')) $virtual_used_bloods = $virtual_used_bloods + 1;
        ...
        // Notify all players
        self::notifyAllPlayers("playerPlayed", clienttranslate('${player_name} writes a ${value} with ${die_color} die in ward n°${num_ward}'), array(
            'player_id' => $player_id,
            'player_name' => self::getCurrentPlayerName(),
            'value' => $value,
    ...
    */
    ...
    function stThrowDice()
    {
        ...
        self::notifyAllPlayers('throwDice', clienttranslate('A new turn begins, new dice are rolled and a new card is flipped'), array(
            ...
            'cards_count' => $this->cards->countCardInLocation('deck') + 1
        ));
