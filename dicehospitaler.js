    ...
    return declare("bgagame.dicehospitaler", ebg.core.gamegui, {
        ...
        */
        ...
        setup: function( gamedatas )
        {
            ...
            for( let player_id in gamedatas.players )
            {
                ...
                if (player['epidemiologist_rooms']) this.displayEpidemiologistRooms(player_id, JSON.parse(player['epidemiologist_rooms']));
                ...
                gamedatas.counters.forEach(type => {
                    dojo.place( this.format_block( 'jstpl_counter', {
                        player_id: player_id,
                        type: type
                    } ) , 'sheet_'+player_id );
                    this.counters[player_id][type] = new ebg.counter() ;
                    this.counters[player_id][type].create('score_'+type+'_'+player_id);
            ...
            for (let player in gamedatas.rooms){
                for (let room in gamedatas.rooms[player]){
                    this.writeANumber(gamedatas.rooms[player][room]['value'], room, player, gamedatas.rooms[player][room]['decoration']);
                }
            }

            for (let player in gamedatas.wards){
                for (let ward in gamedatas.wards[player]){
                    this.writeAWard(ward, player, gamedatas.wards[player][ward]['VP'], gamedatas.wards[player][ward]['nurse']);
                }
            }
            ...
            for (let index in this.dice){
                ...
                dojo.style( this.dice[index], 'background-position-y', this.getYFromDiceNb(gamedatas[this.dice[index]]) );
            ...
            dojo.style( 'radiologist', 'background-position', this.getCoordFromCardNb(gamedatas.radiologist,true) );
            this.addTooltip('radiologist', this.getSpecialistToolTip(gamedatas.radiologist), '');
            dojo.style( 'epidemiologist', 'background-position', this.getCoordFromCardNb(gamedatas.epidemiologist,true) );
            this.addTooltip('epidemiologist', this.getSpecialistToolTip(gamedatas.epidemiologist), '');
            if (gamedatas.cardiologist_1<0){
                dojo.addClass( 'cardiologist_1', "used");
                dojo.style( 'cardiologist_1', 'background-position', this.getCoordFromCardNb(-gamedatas.cardiologist_1,true) );
                this.addTooltip('cardiologist_1', this.getSpecialistToolTip(gamedatas.cardiologist_1), '');
            }else {
                dojo.style( 'cardiologist_1', 'background-position', this.getCoordFromCardNb(gamedatas.cardiologist_1,true) );
                this.addTooltip('cardiologist_1', this.getSpecialistToolTip(gamedatas.cardiologist_1), '');
            }
            if (gamedatas.cardiologist_2<0){
                ...
                dojo.style( 'cardiologist_2', 'background-position', this.getCoordFromCardNb(-gamedatas.cardiologist_2,true) );
                this.addTooltip('cardiologist_2', this.getSpecialistToolTip(gamedatas.cardiologist_2), '');
            }else {
                dojo.style( 'cardiologist_2', 'background-position', this.getCoordFromCardNb(gamedatas.cardiologist_2,true) );
                this.addTooltip('cardiologist_2', this.getSpecialistToolTip(gamedatas.cardiologist_2), '');
            }
            ...
            console.log( "let's try again continuing game setup" );
            ...
            {            
                switch( stateName )
                {
                    case 'playerTurn':
                        ...
                        this.addActionButton( 'blood_all', _('Use blood bag'), 'onUseBlood_all_color' ); 
                    ...
                    case 'otherPlayersTurn':
                        ...
                        this.addActionButton( 'blood_all', _('Use blood bag'), 'onUseBlood_all_color' ); 
                        this.checkBloodAllPossible();
                        this.addActionButton( 'blood_activate', _('Use already played die'), 'onUseBlood_activate' );
        ...
        */
            ...
            prepareAllHexs: function (player_id)
            {
                ...
                for (let room_id in this.gamedatas.sheet['hexs']) {
                    ...
                    let x = this.gamedatas.sheet['hexs_origin_x'] + (column-(row%2==1?0.5:0)) * this.gamedatas.sheet['hexs_offset_x'];
                    let y = this.gamedatas.sheet['hexs_origin_y'] + row * this.gamedatas.sheet['hexs_offset_y'];
                    ...
                    dojo.connect($(div_id),'onclick', this, (e) => { this.onWrite(e,room_id); } );
            ...
            for (let ward_id in this.gamedatas.sheet['wards']) {
                ...
                dojo.place( this.format_block( 'jstpl_ward', {
                    div_id: div_id,
                    X: x,
                    Y: y
                } ) , "sheet_" + player_id );
            ...
            useADie: function (die_id){
                switch (parseInt(die_id)) {
                    case 0:
                        ...
                        this.addTooltip('red_die', _('This die has been used, you can play it though by using a blood bag'), '');
                        this.used_die = "R";
                        break;
                    case 1:
                        dojo.addClass('yellow_die', "used");
                        this.addTooltip('yellow_die', _('This die has been used, you can play it though by using a blood bag'), '');
                        this.used_die = "Y";
                        break;
                    case 2:
                        dojo.addClass('green_die', "used");
                        this.addTooltip('green_die', _('This die has been used, you can play it though by using a blood bag'), '');
                        this.used_die = "G";
                        break;
                    default:
            ...
            updateSquares: function(type, player_id = this.virtual_id){
                ...
                for (let y = 0; y < row; y++) {
                    for (let x = 0; x < column; x++) {
                        ...
                        if (index<=this.gamedatas.players[player_id]['nb_'+type]){
                            dojo.addClass(div_id,"circle");
                        } else if (this.virtual_id == player_id && index<=(this.virtual['nb_'+type]+parseInt(this.gamedatas.players[player_id]['nb_'+type]))){
                        ...
                        if (type=="bloods" || type=="stetos"){
                            if (index<=this.gamedatas.players[player_id]['used_'+type]){
                                dojo.addClass(div_id,"used");
                            } else if (this.virtual_id == player_id && index<=(this.virtual['used_'+type]+parseInt(this.gamedatas.players[player_id]['used_'+type]))){
            ...
            getCoordFromCardNb: function(x,bSpecialist){
                if (bSpecialist){
                    ...
                    let column = x%11;
                    return (-this.specialist_card_width*(column))+"px "+(-this.specialist_card_height*(row))+"px "
                } else {
                    let column = Math.floor(x/7);
                    let row = x%7;
                    return (-this.ambulance_card_width*(column))+"px "+(-this.ambulance_card_height*(row))+"px "
                }
            },
            ...
            updateMoves: function() {
                ...
                if (selected_die!=''){
                    ...
                    if (selected_die[0].id=='extra_die' || selected_die[0].id==this.virtual['extra_die_source']){
                        /* BIG CHANGE
                    ...
                    let power = this.gamedatas.ambulance_infos[this.gamedatas.active_card].value[power_id];
                            ...
                            switch (power) {
                                ...
                                if (power.indexOf("patient")==0){
                                    ...
                                    dojo.style( 'extra_die', 'background-position-y', this.getYFromDiceNb(extra_value) );
                    ...
                    //if selected die, or extra die source is used die, increments used bloods
                        if (this.used_die != '' && (type == this.used_die || this.virtual['extra_die_source'].charAt(0).toUpperCase() == this.used_die)) {
            ...
            toggleExtraDieSource: function(die){
                let other_die = (die=='extra_die')?this.virtual['extra_die_source']:'extra_die';
            ...
            displayPossibleMoves: function(){
                ...
                if (this.virtual['screened_room']!=-1) dojo.addClass("hex_"+this.player_id+"_"+this.virtual['screened_room'], "virtual-screen");
                ...
                for (let ward of this.gamedatas.sheet.wards) {
                    ...
                    for (let room_id of ward.hexs){
                        let value_hex = dojo.attr("hex_"+this.player_id+"_"+room_id,"data-value");
                        //if room is not empty
                        if (value_hex!=0){
                            //if there is a screen, value count as 0
                            if (dojo.hasClass("hex_"+this.player_id+"_"+room_id, "screen") || dojo.hasClass("hex_"+this.player_id+"_"+room_id, "virtual-screen")) {
                                last_room_id = null;
                                last_value = 0;
                            }
                            else {
                            ...
                            if (last_value<value && 
                                (this.virtual['type']=="W" || dojo.attr("hex_"+this.player_id+"_"+room_id,"data-type")==type || dojo.attr("hex_"+this.player_id+"_"+room_id,"data-type")=="W")){
                                dojo.addClass("hex_"+this.player_id+"_"+room_id, "active");
                            }
                            if (decoration == "screen" && last_room_id!=null) dojo.addClass('hex_'+this.player_id+'_'+last_room_id, 'active-screen');
                            break;
                        }
            ...
            rollDice: function(dice){
                for (let index in this.dice){
                    dojo.style( this.dice[index], 'background-position-y', this.getYFromDiceNb(dice[this.dice[index]]) );
            ...
            displayNewCard: function(card_id){
                ...
                if (card_id==-1) {
                    ...
                    dojo.style( 'active_card', 'background-position', this.ambulance_card_width+"px "+this.ambulance_card_height+"px" );
                    return;
                }
                dojo.style( 'active_card', 'background-position', this.getCoordFromCardNb(card_id,false) );
            ...
            getToolTip: function(card_id, num){
                ...
                return this.gamedatas.help[type];
            },
           ...
           set: function(type_value, value, player_id = this.player_id){
            ...
            checkMinusPossible: function(){
                ...
                } else if (this.getSelectedDie()[0].id == 'extra_die' && this.getSelectedValue()%2==0){
                ...
                } else if ((this.get('used_stetos') + this.virtual['used_stetos']) == (this.get('nb_stetos') + this.virtual['nb_stetos']) && this.virtual['modif']<=0){
                    dojo.addClass( 'minus', 'disabled');
            ...
            checkPlusPossible: function(){
                ...
                } else if (this.getSelectedDie()[0].id == 'extra_die' && this.getSelectedValue()%2==1){
            ...
            checkBloodAllPossible: function(){
                ...
                } else if (this.get('used_bloods')+this.virtual['used_bloods']==this.get('nb_bloods')+this.virtual['nb_bloods']){
                    dojo.addClass( 'blood_all', 'disabled');
            ...
            checkBloodActivatePossible: function(){
                ...
                if (this.virtual["blood_activate"]==1) {
                    ...
                    dojo.addClass( 'blood_activate', 'disabled');
                    this.addTooltip('blood_activate', _("You don't have enough blood bags"), '');
                } else {
                    dojo.removeClass('blood_activate', 'disabled');
                    
                    this.addTooltip('blood_activate', '', _('Paying 1 blood bag, you can play the used dice'));
                }
            ...
            update: function(){
                if(this.getSelectedDie()==""){
                    let new_title = bga_format(_('*You* must choose a die'), {
                        '*': (t) => '<span style="font-weight:bold;color:#ff0000;">' + t + '</span>'
                    });
                    ...
                    let new_title = dojo.string.substitute( _('Select a room to write a ${number}'), {
        ...
        */
            ...
            onClickOnDice: function (die){
                ...
                if (dojo.hasClass(die,'used') && die!=this.virtual['extra_die_source']){
                    return;
                }
        ...
        onPass: function (evt){
            ...
            if (this.virtual['first_move']!=''){
                ...
                let decoration = (infos[2]=='extra_die')?"pass_normal_die":"pass_extra_die";

                this.ajaxcall( "/dicehospitaler/dicehospitaler/play.html", { 
                    lock: true, 
                    die: die, 
                    room_id: this.virtual['first_room'], 
                    value: this.virtual['first_value'], 
                    decoration: decoration,
                    extra_die_source : dojo.attr(this.virtual['extra_die_source'],'data-type')
            ...
            this.ajaxcall( "/dicehospitaler/dicehospitaler/pass.html", { 
                lock: true,
        ...
        onWrite: function(evt, room_id, screenOK = false){
            ...
            if (!dojo.hasClass("hex_"+this.player_id+"_"+room_id, 'active') && !dojo.hasClass("hex_"+this.player_id+"_"+room_id, 'active-screen')) return false;
            if (this.getSelectedDie()=='') return;

            if (dojo.hasClass("hex_"+this.player_id+"_"+room_id, 'active-screen')){//if it's just a screen, record the "screen move"
                this.virtual['decoration']='';
                this.virtual['screened_room']= room_id;
                //remove other active-screen
                dojo.query( '.hex.active-screen').removeClass("active-screen");
                this.displayPossibleMoves();
            } else {// else if it's a number AND a screen send it
                ...
                } else { // else if "2 moves needed"
                    let selected_die = this.getSelectedDie()[0];
                    if (selected_die.id=='extra_die' || selected_die.id==this.virtual['extra_die_source']){
                        if (this.virtual['first_move']==''){//record first move
                            let value = $("hex_"+this.player_id+"_"+room_id).innerHTML;

                            this.virtual['first_room'] = room_id;
                            this.virtual['first_value'] = value;
                            this.virtual['first_move'] = this.getSelectedType() + " " + value + " " + selected_die.id;// "R 4 extra_die" or "Y 5 Yellow die"
                            //and reset modif (which has been used)
                            this.virtual['modif'] = 0;

                            let other_die = (selected_die.id=='extra_die')?this.virtual['extra_die_source']:'extra_die';

                            dojo.addClass(selected_die.id, "inactive");
                            this.toggleExtraDieSource(other_die);
                            this.updateMoves();
                        }
                        else {//or send 2 moves
                            this.ajaxcall( "/dicehospitaler/dicehospitaler/play.html", { 
                                lock: true, 
                                die: this.getSelectedType(), //indique quel de a ete choisi
                                room_id: room_id, //indique ou il faut marquer le resultat
                        ...
                        if (this.virtual['decoration']=="screen" && !screenOK){
                            this.confirmationDialog( _("You are using screen and number at same place"), dojo.hitch( this, function() {
        ...
        */
        ...
        notif_scoreWards: function( notif )
        {
            ...
            this.writeAWard(notif.args.ward_id, notif.args.player_id, notif.args.bonus, 1)
        },  
        ...
        notif_scoreNurses: function( notif )
        {
            ...
            this.counters[notif.args.player_id]['nurses'].toValue(notif.args.new_score);
        ...
        notif_scoreCardiologist: function( notif )
        {
            ...
            this.counters[notif.args.player_id][notif.args.cardiologist_id].incValue(notif.args.value);
            this.counters[notif.args.player_id]['total'].incValue(notif.args.value); 
            this.scoreCtrl[ notif.args.player_id ].incValue( notif.args.value );
            ...
            if (notif.args.new_card<0){
                ...
                dojo.style( notif.args.cardiologist_id, 'background-position', this.getCoordFromCardNb(-notif.args.new_card,true) );
                this.addTooltip(notif.args.cardiologist_id, this.getSpecialistToolTip(notif.args.new_card), '');
            } else {
            dojo.style( notif.args.cardiologist_id, 'background-position', this.getCoordFromCardNb(notif.args.new_card,true) );
            this.addTooltip(notif.args.cardiologist_id, this.getSpecialistToolTip(notif.args.new_card), '');
            }
        ...
        notif_scoreRadiologist: function( notif )
        {
            ...
            this.counters[notif.args.player_id]['radiologist'].incValue(notif.args.value);
            this.counters[notif.args.player_id]['total'].incValue(notif.args.value); 
            this.scoreCtrl[ notif.args.player_id ].incValue( notif.args.value );
        ...
        notif_scoreEpidemiologist: function (notif)
        {
            ...
            this.counters[notif.args.player_id]['epidemiologist'].incValue(notif.args.value);
            this.counters[notif.args.player_id]['total'].incValue(notif.args.value); 
            this.scoreCtrl[ notif.args.player_id ].incValue( notif.args.value );
            if (notif.args.epidemiologist_rooms) this.displayEpidemiologistRooms(notif.args.player_id, JSON.parse(notif.args.epidemiologist_rooms));
        ...
        notif_newScreen: function (notif){
            ...
            dojo.addClass("hex_"+notif.args.player_id+"_"+notif.args.room_id, "screen")
        },
        ...
        notif_playerPlayed: function(notif){
            ...
            this.writeANumber(notif.args.value,notif.args.room_id,notif.args.player_id,notif.args.decoration);
            
            this.increase("nb_stetos", notif.args.new_stetos, notif.args.player_id);
            this.increase("used_stetos", notif.args.new_used_stetos, notif.args.player_id);
            this.increase("nb_bloods", notif.args.new_bloods, notif.args.player_id);
            this.increase("used_bloods", notif.args.new_used_bloods, notif.args.player_id);
